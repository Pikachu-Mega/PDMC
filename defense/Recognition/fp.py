...
# ==== Compute-cost logging helpers (common) ====
import csv, time, os, torch, logging
PASS_COUNTER = {"fwd": 0, "bwd": 0}

def reset_pass_counters():
    PASS_COUNTER["fwd"] = 0; PASS_COUNTER["bwd"] = 0
...
def step_end(tag: str, t0: float):
    if torch.cuda.is_available():
        ...
        peak_mem = -1
    dt = time.time() - t0
    logging.info(f"[STEP-END] {tag} | time={dt:.3f}s | peak_mem_MB={peak_mem:.1f} | "
                 f"fwd={PASS_COUNTER['fwd']} | bwd={PASS_COUNTER['bwd']}")
    return dt, peak_mem, PASS_COUNTER["fwd"], PASS_COUNTER["bwd"]

def _csv_path(args, method="baseline"):
    # 统一放到对应方法的 log/checkpoint 目录下（若没有就用 CWD）
    base = getattr(args, "log", None) or getattr(args, "checkpoint_save", None) or os.getcwd()
    os.makedirs(base, exist_ok=True)
    return os.path.join(base, f"metrics_{method}.csv")

def csv_write_header_if_needed(args, method="baseline"):
    p = _csv_path(args, method)
    if not os.path.exists(p):
        with open(p, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["time","method","phase","epoch","clean_acc","asr","ra",
                        "time_sec","peak_mem_MB","fwd","bwd"])

def csv_write_row(args, method, phase, epoch, clean_acc, asr, ra, time_sec, peak_mem_MB, fwd, bwd):
    csv_write_header_if_needed(args, method)
    with open(_csv_path(args, method), "a", newline="") as f:
        w = csv.writer(f)
        w.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), method, phase, epoch,
                    ...
                    fwd if fwd is not None else "", bwd if bwd is not None else ""])
# ===============================================
...
class FinePrune(defense):
    ...
    def set_args(self, parser):
        ...
        parser.add_argument('--result_file', type=str, default="20250819_110004_wanet_attack_prototype_9LWh", help='the location of result')
        parser.add_argument('--yaml_path', type=str, default="D:/zzrubettual/backdoor_bench/config/defense/fp/gtsrb.yaml", help='the path of yaml')
    ...
    def prepare(self, args):
        ...
        '''
                ...
                '''
        # self.attack_result = load_attack_result("D:/zzrubettual/backdoor_bench/record" + os.path.sep + self.args.result_file + os.path.sep +'attack_result.pt')
        # 改为（用 join，再统一替换为正斜杠）
        p = os.path.join("D:/zzrubettual/backdoor_bench/record", self.args.result_file, "attack_result.pt")
        p = p.replace("\\", "/")
        self.attack_result = load_attack_result(p)
    ...
    def defense(self):
        ...
        # === FP: forward scan all clean-train ===
        t0 = step_begin("FP:forward-scan")
        with torch.no_grad():
            flag = 0
        ...
        dt_s, mem_s, fwd_s, bwd_s = step_end("FP:forward-scan", t0)
        csv_write_row(args, method="FP", phase="scan", epoch=None,
                      clean_acc=None, asr=None, ra=None,
                      time_sec=dt_s, peak_mem_MB=mem_s, fwd=fwd_s, bwd=bwd_s)
        ...
        # === FP: fine-tune with per-epoch metrics ===
        last_net.train();last_net.requires_grad_()
        args_print_freq = max(1, len(trainloader) // 5)

        for epoch in range(1, args.epochs + 1):
            t0 = step_begin(f"FP:finetune-epoch={epoch}")
            last_net.train();reset_pass_counters()

            for ib, (img, label, *_) in enumerate(trainloader, start=1):
            ...
            # eval（与 prune 阶段一致）
            clean_acc = given_dataloader_test(last_net, clean_test_dataloader, criterion,
                                              args.non_blocking, args.device)[0]['test_acc'] * 100.0
            asr = given_dataloader_test(last_net, bd_test_dataloader, criterion,
                                        args.non_blocking, args.device)[0]['test_acc'] * 100.0
            ra = 100.0 - asr

            dt, mem, fwd, bwd = step_end(f"FP:finetune-epoch={epoch}", t0)
            csv_write_row(args, method="FP", phase="finetune", epoch=epoch,
                          clean_acc=clean_acc, asr=asr, ra=ra,
                          time_sec=dt, peak_mem_MB=mem, fwd=fwd, bwd=bwd)
