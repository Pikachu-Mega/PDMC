...
class dsam(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--checkpoint_save', type=str, default='D:/zzrubettual/backdoor_bench/record0821',
                            metavar='N',
                            help='number of curve bends (default: 3)')
        parser.add_argument('--log', type=str, default='D:/zzrubettual/backdoor_bench/record0821',
    ...
    def set_result(self, result_file):
        attack_file = 'D:/zzrubettual/backdoor_bench/record/' + result_file
    ...
    def _train_sam(self, args, train_loader, model, optimizer, scheduler,criterion, epoch):
        ...
        for idx, (img, target, *flag) in enumerate(train_loader, start=1):
            ...
            # SAM 每步包含 ascent 与 descent 两个前后向（实现细节在 optimizer 内）
            inc_fwd(2);inc_bwd(2)
    ...
    def train_sam(self, model,train_dataloader,
        ...
        for epoch in tqdm(range(1, args.epochs+1)):
            t0 = step_begin(f"FT-SAM:epoch={epoch}")
            reset_pass_counters()
            ...
            # 结束 & 记 CSV
            dt, mem, fwd, bwd = step_end(f"FT-SAM:epoch={epoch}", t0)
            # _train_sam 里每 step 做了 SAM 的 ascent+descent，两次前反向；近似统计：
            # （也可以在 _train_sam 内精确 inc_fwd/inc_bwd，见下一个补丁）
            csv_write_row(args, method="FT-SAM", phase="train", epoch=epoch,
                          clean_acc=test_acc * 100.0, asr=test_asr * 100.0, ra=test_ra * 100.0,
